...
def create_dataset(strain, segment_duration, signal_func, **kwargs):
    results = []
    ...
    for chunk_start, chunk_end in tqdm(chunks_idxs):
...
def process_file(strain_file, detector):
    ...
    for sig_func in signal_functions:
        print(f"building dataset for {sig_func.__name__}:")
        sig_dataset = create_dataset(strain.copy(), segment_duration, sig_func, detector=detector)
